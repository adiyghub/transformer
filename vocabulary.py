...
class LetterVocabulary:
    ...
    def build_vocabulary(self, source_file='train.en', target_file='train.de'):
        ...
        with io.open('train.en', encoding="utf8") as f1, io.open('train.de', encoding="utf8") as f2:
                for string_en, string_de in zip(f1, f2):
                    self.source_target_list.append([" ".join(string_en.split()), " ".join(string_de.split())])
        ...
        self.ch2ix.update(
            {
                x[0]: i
                for i, x in enumerate(letter_counts)
                }
            )
        ...
        self.ch2ix.default_factory = lambda: self.vocab_size # set default_factory to vocab_size
